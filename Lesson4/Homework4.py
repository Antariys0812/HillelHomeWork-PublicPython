...
number = 0
for k in adwentures_of_tom_sawer.split():
    if k.istitle():
        number += 1
print(number)
...
"""
print(adwentures_of_tom_sawer.replace('Tom', 'Ton', 1).find('Tom'))
...
"""
kilkist_bythetime = 0
for k in adwentures_of_tom_sawer_sentences:
    if k.startswith("By the time"):
        kilkist_bythetime += 1
        print(f'Це речення починається з "By the time": {k}')

if kilkist_bythetime == 0:
    print(f'No речення починається з "By the time" :(')
