...
def car_find():
    ...
    for key, value in car_data.items():
        if value[1] >= 2017 and value[2] >= 1.6 and value[4] <= 36000:
            car_data_found[key] = value
    print(car_data_found)
    return car_data_found

car_data_found = car_find()

def to_sort(item):
    return item[1][4]

car_data_found = sorted(car_data_found.items(), key = to_sort)
print(car_data_found[:5])
...
people_records.insert(0, ('Iryna', 'Kuzmych', 22, 'Data Analyst', 'Kyiv'))
zminna = people_records[5]
people_records[5] = people_records[1]
people_records[1] = zminna
print(people_records)

if people_records[6][2] >=30 and people_records[10][2] >=30 and people_records[13][2] >=30:
    print(f'Ось ці люди старші за 30 років: {people_records[6][2]}, {people_records[10][2]}, {people_records[13][2]}')
else:
    print('На цих індексах немає людей старше 30 років')
